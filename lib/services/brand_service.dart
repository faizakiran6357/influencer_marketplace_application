...
//   /// ✅ Fetch brand from `profiles` table
...
//       if (res['role'] != null &&
//           res['role'].toString().toLowerCase() != 'brand') {
//         print("⚠️ User role is not brand: ${res['role']}");
//         return null;
//       }
...
//     try {
//       await _client.from('profiles').update(brand.toMap()).eq('id', brand.id);
//     } catch (e) {
...
//     try {
//       final path = 'brand_profiles/$brandId.jpg';
//       await _client.storage.from('profile_pics').upload(path, imageFile, fileOptions: const FileOptions(upsert: true));
...
//   // Fetch brand data from profiles table by user id
//   Future<BrandModel?> fetchBrandByUserId(String userId) async {
//     try {
//       final res = await _client
//           .from('profiles')
...
//   // Update brand profile details in Supabase
//   Future<void> updateBrand(BrandModel brand) async {
//     try {
//       await _client.from('profiles').update(brand.toMap()).eq('id', brand.id);
//     } catch (e) {
...
//   Future<String?> uploadProfileImage(String brandId, File imageFile) async {
//     const bucketName = 'profile_images'; // must match your Supabase bucket name
//     try {
//       final fileBytes = await imageFile.readAsBytes();
//       final filePath = 'brands/$brandId-${DateTime.now().millisecondsSinceEpoch}.jpg';
...
//       // Update the profiles table with new image URL
//       await _client.from('profiles').update({
//         'profile_image': publicUrl,
//         'updated_at': DateTime.now().toIso8601String(),
//       }).eq('id', brandId);

//       print("✅ Brand profile image uploaded: $publicUrl");
//       return publicUrl;
//     } on StorageException catch (e) {
//       print("❌ StorageException during uploadProfileImage: ${e.message}");
//       return null;
//     } catch (e) {
//       print("❌ uploadProfileImage error: $e");
//       return null;
